...
var slots_fifa = [];
var slots_flight_simulator = [];
var slots_mario_kart = [];
var slots_mortal_kombat = [];
...
async function GetTimeslots(){
    ...
    if(response.status == 200){
        ...
        for(var i=0; i<responseData.flight_simulator.length; i++){
            slots_flight_simulator.push(responseData.flight_simulator[i]);
        }
        for(var i=0; i<responseData.mario_kart.length; i++){
            slots_mario_kart.push(responseData.mario_kart[i]);
        }
        for(var i=0; i<responseData.mortal_kombat.length; i++){
            slots_mortal_kombat.push(responseData.mortal_kombat[i]);
        }
...
socket.on('timeslot', function(response){
    switch(response.Activity){
        ...
        case 'fifa':
            ...
            break;
        case 'flight-simulator':
            slots_flight_simulator.push(parseInt(response.Timeslot));
            break;
        case 'mario-kart':
            slots_mario_kart.push(parseInt(response.Timeslot));
            break;
        case 'mortal-kombat':
            slots_mortal_kombat.push(parseInt(response.Timeslot));
            break;
        case 'rocket-league':
